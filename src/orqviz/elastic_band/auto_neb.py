...
def run_AutoNEB(
    ...
    """Automatic wrapping of the NEB algorithm that dynamically inserts pivots to the piece-wise linear path being optimized in the loss function landscape.
    Returns a list of all chains during training where index -1 is the final optimized chain.
    ...
    Args:
        ...
        n_cycles: Number of cycles between which new pivots can be inserted. Defaults to 4.
        n_iters_per_cycle: Number of optimization iterations per cycle. Defaults to 10.
        max_new_pivots: Maximum number of pivots inserted per cycle. Defaults to 1.
        weighted_redistribution: Flag to indicate whether pivots are uniformly re-distributed along the chain or according to their insertion position in the chain. Defaults to False.
        insert_at_beginning: Flag to indicate whether to insert pivots before first cylce. Defaults to False.
        percentage_tol: Percentage error threshold to insert new pivots. Called 'alpha' in the original paper.
            Be mindful of the magnitude and sign of typical loss values. Additive to absolute error. Defaults to 0.2.
        absolute_tol: Absolute error threshold to insert new pivots. Additive to percentage error. Defaults to 0.0.
        eps: Stencil for finite difference gradient if gradient_function is not provided. Defaults to 0.1.
        learning_rate: Learning rate/ step size for the gradient descent optimization. Defaults to 0.1.
        stochastic: Flag to indicate whether to perform stochastic gradient descent if gradient_function is not provided. It is less stable but much faster. Defaults to False.
...
def _insert_pivots_to_improve_approximation(
    ...
    """Method to check where the piece-wise linear approximation of the current Chain with respect to the loss function is not good enough. If that's the case, it adds new pivots to the chain.

    Args:
        chain: Current Chain
        loss_function: Loss function for the NEB training
        max_new_pivots: Maximum number of pivots inserted to Chain. Defaults to 1.
        percentage_tol: Percentage error threshold to insert new pivots. Be mindful of the magnitude and sign of typical loss values. Additive to absolute error. Defaults to 0.2.
        absolute_tol: Absolute error threshold to insert new pivots. Additive to percentage error. Defaults to 0.0.
    """
    ...
    # Check between each pair of pivots if an additional pivot would improve the approximation
    trial_cum_weights = [
    ...
    if not np.isclose(significant_residuals, 0).all():
        ...
        )  # sort the most important indices backwards to insert correctly into the chain
